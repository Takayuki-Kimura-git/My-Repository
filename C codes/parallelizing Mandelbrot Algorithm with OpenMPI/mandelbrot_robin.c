...
int main(int argc, char** argv)
 {  double overall =  MPI_Wtime();
	...
	if(rank == 0){
		...
		for(iY = 0; iY < iYmax; iY+=processors)
		{
		...
		if(rank < screen_remain){
			result_offset += (screen_per_procs +1) * iXmax * 3;
		}else{
			//printf("process %d screen is %d iXmax is %d .\n",i,screen_per_procs,iXmax);
			result_offset += screen_per_procs * iXmax * 3;
			//printf("process %d offset is %d  .\n",rank,result_offset);
		}
		for(i=1; i<processors; i++){
			//printf("process %d offset is %d  .\n",i,result_offset);
			if(i < screen_remain){
				//printf("process %d start receiving remeinder.\n",i);
				MPI_Recv(temp+result_offset,(screen_per_procs +1) * iXmax * 3, MPI_UNSIGNED_CHAR, i, 0, MPI_COMM_WORLD, &stat);
				result_offset+=(screen_per_procs+1) * iXmax * 3;
			
				//fwrite(temp,1,screen_per_procs * iXmax * 3,fp);
			}else{
				MPI_Recv(temp +result_offset , (screen_per_procs) * iXmax * 3, MPI_UNSIGNED_CHAR, i, 0, MPI_COMM_WORLD, &stat);
				result_offset+=screen_per_procs * iXmax * 3;
		...
		for(i=0;i<screen_per_procs;i++){
			for(j=0;j<processors;j++){
				fwrite(temp+i*iXmax*3+(j*screen_per_procs)*iXmax*3,1,iXmax*3,fp);
				//printf("process %d successfully finish fwrite.\n",i);
			}
		}
		for(i=0;i<screen_remain;i++){
			for(j=0;j<screen_remain;j++){
				fwrite(temp+screen_per_procs*iXmax*3+(j*screen_per_procs)*iXmax*3,1,iXmax*3,fp);
			}
		}
		...
		/* compute and write image data bytes to the file */
		//IYmax, IXmax are 8000 default
		for(iY = rank; iY < iYmax; iY+=processors)
		{
		...
		if(rank < screen_remain){
			//printf("process %d start sending remeinder.\n",rank);
			MPI_Send(temp,(screen_per_procs+1) * iXmax * 3 , MPI_UNSIGNED_CHAR, 0, 0, MPI_COMM_WORLD); 
		}else{
			//printf("process %d start sending.\n",rank);
			MPI_Send(temp, (iYmax/processors) * iXmax * 3 , MPI_UNSIGNED_CHAR, 0, 0, MPI_COMM_WORLD);
